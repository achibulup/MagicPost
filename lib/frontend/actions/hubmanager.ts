...
export async function fetchOrders() {
  const result = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/hubmanager/orders`);
...
export async function fetchEmployees() {
  const result = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/hubmanager/employees?role=staff`);
  if (Math.floor(result.status / 100) !== 2) throw new Error((await result.json()).error);
  return await result.json() as AccountExtended[];
}

export async function createEmployee(postform: FormData) {
  const result = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/hubmanager/employees`, {
    method: 'POST',
    body: postform
  });
...
export async function changeEmployee(employeeId: number, postform: FormData) {
  const result = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/hubmanager/employees/${employeeId}`, {
    method: 'PATCH',
    body: postform
  });
  if (Math.floor(result.status / 100) === 2) {
