...
export async function getEmployees(filter: EmployeeFilter) {
  ...
  if (!doFilterPickupPoint && !doFilterTransitHub) {
    const ppEmployees = await sql<Account>`
      SELECT a."id", a."name", a."email", a."phone", a."role", a."pickupPoint", a."transitHub", pp."name" as "at" 
      FROM "accounts" a
      JOIN "pickupPoints" pp ON pp."id" = a."pickupPoint"
      WHERE 
        (${doFilterName} = 0 OR a."name" = ${name}) AND
        (${doFilterEmail} = 0 OR "email" = ${email}) AND
        (${doFilterRole} = 0 OR "role" = ${role})
    `.then((res) => res.rows);
    const thEmployees = await sql<Account>`
      SELECT a."id", a."name", a."email", a."phone", a."role", a."pickupPoint", a."transitHub", th."name" as "at" 
      FROM "accounts" a
      JOIN "transitHubs" th ON th."id" = a."transitHub"
      WHERE 
        (${doFilterName} = 0 OR a."name" = ${name}) AND
        (${doFilterEmail} = 0 OR "email" = ${email}) AND
        (${doFilterRole} = 0 OR "role" = ${role})
    `.then((res) => res.rows);
    return [...ppEmployees, ...thEmployees];
  }
  return sql<Account>`
    SELECT a."id", a."name", a."email", a."phone", a."role", a."pickupPoint", a."transitHub" FROM "accounts" a
    WHERE 
